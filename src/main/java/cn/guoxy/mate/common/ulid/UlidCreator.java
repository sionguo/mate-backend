package cn.guoxy.mate.common.ulid;
 ...
 *
 * <p>Both types of ULID can be easily created by this generator, i.e. monotonic and non-monotonic.
 *
 * <p>In addition, a "non-standard" hash-based ULID can also be generated, in which the random
...
public final class UlidCreator {
   ...
   * Returns a ULID.
   *
   * <p>The random component is reset for each new ULID generated.
   *
   * @return a ULID
   */
   ...
   *
   * <p>The random component is incremented for each new ULID generated in the same millisecond.
   *
   * @return a ULID
   */
   ...
   * Returns a Hash ULID.
   *
   * <p>The random component is replaced with the first 10 bytes of an SHA-256 hash.
   *
   * <p>It always returns the same ULID for a specific pair of {@code time} and {@code string}.
   *
   * <p>Usage example:
   *
   * <pre>{@code
   ...
   * <p>The random component is replaced with the first 10 bytes of an SHA-256 hash.
   *
   * <p>It always returns the same ULID for a specific pair of {@code time} and {@code bytes}.
   *
   * <p>Usage example:
   *
   * <pre>{@code
