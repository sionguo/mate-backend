 ...
 *
 * <p>ULID is a 128-bit value that has two components:
 *
 * <ul>
 *   <li><b>Time component</b>: a number of milliseconds since 1970-01-01 (Unix epoch).
 *   <li><b>Random component</b>: a sequence of 80 random bits generated by a secure random
 *       generator.
 * </ul>
 *
 * <p>ULID has 128-bit compatibility with {@link UUID}. Like a UUID, a ULID can also be stored as a
 * 16-byte array.
 *
...
public final class Ulid implements Serializable, Comparable<Ulid> {
   ...
   * Creates a new ULID.
   *
   * <p>Useful to make copies of ULIDs.
   *
   * @param ulid a ULID
   */
   ...
   * Creates a new ULID.
   *
   * <p>If you want to make a copy of a {@link UUID}, use {@link Ulid#from(UUID)} instead.
   *
   ...
   * Creates a new ULID.
   *
   * <p>The time parameter is the number of milliseconds since 1970-01-01, also known as Unix epoch.
   * It must be a positive number not larger than 2^48-1.
   *
   * <p>The random parameter must be an arbitrary array of 10 bytes.
   *
   * <p>Note: ULIDs cannot be composed of dates before 1970-01-01, as their embedded timestamp is
   * internally treated as an unsigned integer, i.e., it can only represent the set of natural
   * numbers including zero, up to 2^48-1.
   *
   ...
   *
   * <p>This static method is a quick alternative to {@link UlidCreator#getUlid()}.
   *
   * <p>It employs {@link ThreadLocalRandom} which works very well, although not cryptographically
   ...
   * generator should use {@link UlidCreator#getUlid()}.
   *
   * @return a ULID
   * @since 5.1.0
   */
   ...
   *
   * <p>The 48 bits of the time component are filled with the given time and the 80 bits of the
   * random component are all set to ZERO.
   *
   * <p>For example, the minimum ULID for 2022-02-22 22:22:22.222 is `{@code new
   * Ulid(0x017f2387460e0000L, 0x0000000000000000L)}`, where `{@code 0x017f2387460e}` is the
   * timestamp in hexadecimal.
   *
   * <p>It can be useful to find all records before or after a specific timestamp in a table without
   * a `{@code created_at}` field.
   *
   * @param time the number of milliseconds since 1970-01-01
   * @return a ULID
   * @since 5.2.0
   ...
   *
   * <p>The 48 bits of the time component are filled with the given time and the 80 bits or the
   * random component are all set to ONE.
   *
   * <p>For example, the maximum ULID for 2022-02-22 22:22:22.222 is `{@code new
   * Ulid(0x017f2387460effffL, 0xffffffffffffffffL)}`, where `{@code 0x017f2387460e}` is the
   * timestamp in hexadecimal.
   *
   * <p>It can be useful to find all records before or after a specific timestamp in a table without
   * a `{@code created_at}` field.
   ...
   *
   * <p>The input string must be 26 characters long and must contain only characters from
   * Crockford's base 32 alphabet.
   *
   * <p>The first character of the input string must be between 0 and 7.
   *
   ...
   *
   * <p>The time component is a number between 0 and 2^48-1. It is equivalent to the count of
   ...
   *
   * <p>The random component is an array of 10 bytes (80 bits).
   *
   * @param string a canonical string
   * @return a byte array
   * @throws IllegalArgumentException if the input string is invalid
   ...
   *
   * <p>A ULID has 128-bit compatibility with a {@link UUID}.
   *
   * <p>If you need a RFC-4122 UUIDv4 do this: {@code Ulid.toRfc4122().toUuid()}.
   *
   * @return a UUID.
   */
  public UUID toUuid() {
   ...
   *
   * <p>The output string is 26 characters long and contains only characters from Crockford's Base
   * 32 alphabet.
   *
   * <p>For lower case string, use the shorthand {@code Ulid#toLowerCase()}, instead of {@code
   ...
   * 32 alphabet.
   *
   * <p>It is a shorthand at least twice as fast as {@code Ulid.toString().toLowerCase()}.
   *
   * @return a string
   * @see <a href="https://www.crockford.com/base32.html">Crockford's Base 32</a>
   */
   ...
   *
   * <p>The bytes of the returned ULID are compliant with the RFC-4122 version 4.
   *
   * <p>If you need a RFC-4122 UUIDv4 do this: {@code Ulid.toRfc4122().toUuid()}.
   *
   * <p><b>Note:</b> If you use this method, you can not get the original ULID, since it changes 6
   * bits of it to generate a UUIDv4.
   *
   * @return a ULID
   ...
   *
   * <p>Since the random component contains 80 bits:
   *
   * <ul>
   *   <li>(1) This method can generate up to 1208925819614629174706176 (2^80) ULIDs per
   *       millisecond;
   *   <li>(2) This method can generate monotonic increasing ULIDs 99.999999999999992% ((2^80 -
   *       10^9) / (2^80)) of the time within a single millisecond interval, considering an
   *       unrealistic rate of 1,000,000,000 ULIDs per millisecond.
   * </ul>
   *
   * <p>Due to (1) and (2), it does not throw the error message recommended by the specification.
   * When an overflow occurs in the random 80 bits, the time component is simply incremented to
   * <b>maintain monotonicity</b>.
   *
   * @return a ULID
   */
   ...
   *
   * <p>The first of two ULIDs is greater than the second if the most significant byte in which they
