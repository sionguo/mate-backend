 ...
 *
 * <p>This class is used by {@link KsuidCreator}.
 *
 * <p>You can use this class if you need to use a specific random generator strategy. However, most
 * people just need {@link KsuidCreator}.
 *
 * <p>Instances of this class can behave in one of three ways: non-monotonic (Segment's), monotonic
 * and sub-second.
 *
 * <p>If the factory is monotonic, the random component is incremented by 1 If more than one KSUID
 * is generated within the same second.
 *
 * <p>If the factory is sub-second, the time precision is detected at runtime.
 *
 * <p>The maximum KSUID that can be generated per second is 2^128.
 */
public final class KsuidFactory {
  ...
  KsuidFactory(Function<Instant, Ksuid> ksuidFunction, Supplier<Instant> instantFunction) {
    ...
    this.instantFunction =
        instantFunction != null ? instantFunction : () -> Clock.systemUTC().instant();
  }

  /**
   * Returns a new Segment's KSUID factory.
   *
   * <p>It is equivalent to {@code new KsuidFactory()}.
   *
   ...
   */
  public static KsuidFactory newSubsecondInstance(
      Random random, Supplier<Instant> instantFunction) {
   ...
   */
  public static KsuidFactory newSubsecondInstance(
      LongSupplier randomFunction, Supplier<Instant> instantFunction) {
   ...
   */
  public static KsuidFactory newSubsecondInstance(
      IntFunction<byte[]> randomFunction, Supplier<Instant> instantFunction) {
  ...
  public static KsuidFactory newMonotonicInstance(
      LongSupplier randomFunction, Supplier<Instant> instantFunction) {
    return new KsuidFactory(
        new MonotonicFunction(IRandom.newInstance(randomFunction)), instantFunction);
  }
